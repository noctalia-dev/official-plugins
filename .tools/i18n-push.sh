...
PROJECT_SLUG="official-plugins"
OVERWRITE=false
SINGLE_LANG=""

usage() {
    echo "Usage: OFFICIAL_PLUGINS_PUSH_SECRET=... $0 [--overwrite] [--lang <locale>]"
}
...
if [[ -z "${OFFICIAL_PLUGINS_PUSH_SECRET:-}" ]]; then
    echo "Error: OFFICIAL_PLUGINS_PUSH_SECRET is required" >&2
    exit 1
fi
...
HTTP_CODE="$(curl --silent --show-error \
    ...
    --write-out '%{http_code}' \
    --request POST \
    --header "Authorization: Bearer $OFFICIAL_PLUGINS_PUSH_SECRET" \
